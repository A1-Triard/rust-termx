...
impl Render {
    ...
    pub fn visual_children_count_impl(this: &Rc<dyn IsRender>, entity: Entity, world: &mut World) -> usize {
        let render = this.render();
        if let Some(decorator) = entity.component::<Decorator>(world, render.decorator) {
    ...
    pub fn add_visual_child_impl(this: &Rc<dyn IsRender>, parent: Entity, child: Entity, world: &mut World) {
        let render = this.render();
        child.component::<View>(world, render.view).unwrap().visual_parent = Some(parent);
    ...
    pub fn remove_visual_child_impl(
        ...
        this.invalidate_render(child, world);
        let render = this.render();
        let mut view = child.component::<View>(world, render.view).unwrap();
